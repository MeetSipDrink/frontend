...
const Roulette = () => {
    ...
    const spin = () => {
        ...
        const randomSpin = ran * arc + baseRotation + extraRotation + correctionOffset;
                ...
                slices.push(
                    <G key={index}>
                        ...
                        <SvgText
                            ...
                            textAnchor="middle"
                            alignmentBaseline="middle"
                            transform={`rotate(${90 + startAngle + sliceAngle / 2}, ${CIRCLE_SIZE / 2 + (CIRCLE_SIZE / 3) * Math.cos(Math.PI * (startAngle + sliceAngle / 2) / 180)}, ${CIRCLE_SIZE / 2 + (CIRCLE_SIZE / 3) * Math.sin(Math.PI * (startAngle + sliceAngle / 2) / 180)})`}
                        >
                            {item}
                        </SvgText>
                    </G>
            ...
            <View style={styles.wheelContainer}>
                <Animated.View style={[styles.wheel, { transform: [{ rotate: rotation }] }]}>
                    <Svg height={CIRCLE_SIZE} width={CIRCLE_SIZE}>
                        <Circle cx={CIRCLE_SIZE / 2} cy={CIRCLE_SIZE / 2} r={CIRCLE_SIZE / 2 - 5} fill="none" stroke="#ffffff" strokeWidth="1" />
                        {renderSlices()}
                    </Svg>
                </Animated.View>
